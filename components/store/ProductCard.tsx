...
import { ShoppingCart, Check, Monitor, Apple } from "lucide-react";
...
export default function ProductCard({ product, currency }: ProductCardProps) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (data.url) {
        window.location.href = data.url; // Stripe or Razorpay redirect
      }
    } catch (error) {
    ...
    >
      ...
      <div className="p-5 4k:p-10 8k:p-20 flex flex-col flex-grow">
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 px-4 4k:py-6 4k:px-8 8k:py-12 8k:px-16 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl 4k:rounded-2xl 8k:rounded-[3rem] font-semibold text-base 4k:text-3xl 8k:text-5xl flex items-center justify-center gap-2 4k:gap-5 8k:gap-10 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {loading ? (
            <span className="animate-spin h-5 w-5 4k:w-10 4k:h-10 8k:w-20 8k:h-20 border-2 4k:border-4 8k:border-8 border-white border-t-transparent rounded-full" />
          ) : (
            <>
